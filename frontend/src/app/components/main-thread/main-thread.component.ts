...
export class MainThreadComponent implements OnInit, OnDestroy {
    ...
    public sliderChangedLike(event: MatSlideToggleChange) {
        if (event.checked) {
            this.isOnlyMine = true;
            this.likePosts.length = 0;
            this.cachedPosts.forEach(element => {
                if (element.reactions.filter((x) => x.user.id === this.currentUser.id && x.isLike === true).length !== 0) {
                    this.likePosts.push(element);
                }
